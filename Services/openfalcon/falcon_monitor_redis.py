...
# 请自己修改下面的endpoint    password
password='redis_password'
appcenterList = {
    'endpoint':['10.0.0.248','6379',password],
}
falconList = {
...
def main():
    for project,hostGroup in  projectList.items():
        if len(hostGroup) == 0:
            continue
        for endpoint,host in hostGroup.items():
            tags='project='+project+',port='+host[1]+',app=redis'
            payload=GetInfo(endpoint,host,tags)
            pushData(payload)
    
def pushData(payload):
	# 请修改成自己的openfalcon-agent 的 http 端口
    r = requests.post("http://127.0.0.1:19080/v1/push", payload)
    print r.text
